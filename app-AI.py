...
from concurrent.futures import ThreadPoolExecutor  # ✅ 新增：多執行緒
...
def build_ai_prompt(data_str, taiex_price, contract_info, data_date):
    ...
    prompt = f"""
你是一位專業的台指期 / 選擇權交易員助理。注意：本 prompt 的資料由系統端【已經判斷並過濾】為「下一個即將結算的合約」資料（包含週三 / 週五 / 月選第三週三的規則），**請勿重新推斷或更改結算日**。若你發現資料日期與系統標注的結算日不一致，請直接回報「資料日期異常」而非自行假設。
