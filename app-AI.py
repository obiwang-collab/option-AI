...
def plot_tornado_chart(df_target, title_text, spot_price):
    ...
    annotations.append(dict(
        ...
        bgcolor="white", bordercolor="#2ca02c",
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            tickmode='array',
            tickvals=[
                -x_limit*0.75, -x_limit*0.5, -x_limit*0.25,
                0,
...
def build_ai_prompt(data_str, taiex_price, contract_info):
    ...
    prompt = f"""
你是台指期市場的『冷血控盤莊家』(Market Maker)。
你的目標是：**讓大多數散戶賠錢，讓賣方利潤最大化 (Max Pain)**。
...
1. 回答只能使用繁體中文。
2. 請用條列清楚說明，避免廢話。
3. 禁止輸出任何程式碼、Python、Streamlit、DeltaGenerator、main()、BlockPath_pb2、
   Creator of Delta protobuf messages 等技術說明文字。
4. 不要解釋你是 AI、不要道歉、不要自我介紹。
...
🩸 **散戶獵殺區 (Kill Zone)**：
（指出哪個價位的 Call 或 Put 散戶最多？如果拉過去或殺下去，誰會斷頭？）

☠️ **控盤劇本**：
（偏多誘空？還是拉高出貨？還是區間盤整吃權利金？請直接給出你的邪惡計畫）
...
def ask_gemini(prompt_text):
    ...
    try:
        ...
        return clean_ai_output(res.text)
    except Exception as e:
...
def ask_chatgpt(prompt_text):
    ...
    try:
        response = openai_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a ruthless market maker. Reply in Traditional Chinese only, no code, no technical documentation."},
...
def main():
    ...
    if st.button("🧛‍♂️ 啟動莊家思維分析", type="primary"):
        if not gemini_model and not openai_client:
            ...
            with col1:
                ...
                if gemini_model:
                    st.info(gemini_result) if gemini_result else st.warning("無回應")
                else:
                    st.warning("未設定 Key")
    ...
    c1.markdown(
        ...
        f"<span style='font-size: 18px; font-weight: bold;'>{datetime.now(tz=TW_TZ).strftime('%Y/%m/%d %H:%M:%S')}</span>"
        "</div>",
